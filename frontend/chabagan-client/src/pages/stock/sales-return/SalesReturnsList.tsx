...
import { useDeleteSalesReturnMutation, useGetSalesReturnsQuery } from "../../../redux/features/stock/salesReturnApi";
...
export default function SalesReturnsList() {
    ...
    const { data, isSuccess } = useGetSalesReturnsQuery(null);
    const [deleteSalesReturn, { isSuccess: isDeleteSuccess, data: deleteData, error: deleteError }] = useDeleteSalesReturnMutation();
    ...
    const onDownloadClick = (row: any) => {
        let viwerInfo: IPdfViwerModel = {
            ...
            url: `${ApiBaseURL}salesreturns/invoice?id=${row.id}`,
            open: true
        }
        setViwer(viwerInfo);
    }
    ...
    const onEditClick = (row: GridCellParams) => {
        navigate(`/stock/sales-returns/edit-sales-returns/${row.id}`);
    }
    ...
    const handleAddClick = () => {
        navigate('/stock/sales-returns/new-sales-returns');
    }

    useEffect(() => {
        document.title = `Sales Returns | ${ProjectTitle}`;
    }, []);
    ...
    return (
        <>
            ...
            <Grid item xs={12} sm={12} md={12} mt={2} className="d-block">
                <Button variant="contained" className="pull-right" onClick={handleAddClick}><AddIcon /> &nbsp; New Sales Return</Button>
            </Grid>

            <Grid item xs={12} sm={12} md={12} mt={2}>
                <LoadDataGrid title="Sales Returns" rows={rows} columns={columns} />
            </Grid>
